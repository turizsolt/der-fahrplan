<template>
  <div class="overflow-y">
    <div>Trip Connector</div>
    Last:
    <select v-model="lastRoutes" multiple @change="handleLastRoutesChanged">
      <option
        v-for="route in lastRouteOptions"
        :value="route.id"
        :key="route.id"
      >
        {{ route.name }} {{ route.detailedName }}
      </option>
    </select>
    <br />
    First:
    <select v-model="firstRoutes" multiple @change="handleFirstRoutesChanged">
      <option
        v-for="route in firstRouteOptions"
        :value="route.id"
        :key="route.id"
      >
        {{ route.name }} {{ route.detailedName }}
    ...
    <div>
      ...
      <input
        type="checkbox"
        v-model="showUnattachedOnly"
        @changed="handleShowUnattachedOnly"
      />
      Show unattached only
      <input
        type="checkbox"
        v-model="showAttachedOnly"
        @changed="handleShowAttachedOnly"
      />
      Show attached only
    </div>
    <hr />
    <table>
      <tr class="xyz" :key="t.key" v-for="t in tripsWithEnd">
        ...
        <td v-if="t.Last" class="blue box" @click="copy(t.Last.trip.id)">
          {{ t.Last.trip.nextTrip }}
        </td>

        <td v-if="!t.Last || !t.Last.trip.nextTrip"></td>
        <td
          v-if="t.Last && t.Last.trip.nextTrip"
          class="box"
          @click="toggleReverse(t.Last.trip.id)"
        >
          {{ t.Last.trip.nextReverse ? "R" : "-" }}
        </td>
        ...
        <td
          v-if="t.Last && t.Last.trip.nextTrip"
          class="red box"
          @click="detach(t.Last.trip.id)"
        >
          detach
        </td>

        <td v-if="!t.First" colspan="5"></td>
        <td v-if="t.First" class="blue box" @click="paste(t.First.trip.id)">
          {{ t.First.trip.prevTrip }}
        </td>
...
export default class TripConnector extends Vue {
  ...
  get tripsWithEnd(): Record<TripEnd, TripWithEnd>[] {
    const lasts = this.tripList.filter((t) =>
      this.lastRoutes.includes(t.route.id)
    );
    const firsts = this.tripList.filter((t) =>
      this.firstRoutes.includes(t.route.id)
    );
    const all = [];
    all.push(
      ...lasts.map((t) => ({
  ...
  update(): void {
    ...
    this.commonStationName = (
      getStorable(this.commonStation) as Station
    )?.getName();

    this.firstRouteOptions = getAllOfStorable(TYPES.Route)
      .filter(
        (x: Route) =>
          !this.commonStation ||
          x.getFirstStation()?.getId() === this.commonStation
      )
      .map((x) => Object.freeze(x.persistDeep()));

    this.lastRouteOptions = getAllOfStorable(TYPES.Route)
      .filter(
        (x: Route) =>
          !this.commonStation ||
          x.getLastStation()?.getId() === this.commonStation
      )
      .map((x) => Object.freeze(x.persistDeep()));
      */
  }
    ...
    for (let i = 0; i < all.length; i++) {
      if (all[i].end === TripEnd.Last && !all[i].trip.nextTrip) {
        for (let j = i + 1; j < all.length; j++) {
          ...
          if (
            all[j].end === TripEnd.First &&
            !all[j].trip.prevTrip &&
            all[j].time >= all[i].time + min
          ) {
