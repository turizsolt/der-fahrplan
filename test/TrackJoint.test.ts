...
describe('Track joint - equation', () => {
  ...
  it('a = 1 equ', () => {
    const j = newTrackJoint(4, 2, (45 / 180) * Math.PI);
    expect(j.slope()).almost(1);
    expect(j.equ()).deep.almost({ a: 1, b: 2 });
  });

  it('a = 2 equ', () => {
    const j = newTrackJoint(5, 2, Math.atan2(4, 2));
    expect(j.slope()).almost(2);
    expect(j.equ()).deep.almost({ a: 2, b: 1 });
  });

  it('a = Inf equ', () => {
    const j = newTrackJoint(5, 2, Math.atan2(4, 0));
...
describe('Track joint - define points', () => {
  ...
  it('aligned on z axis', () => {
    const j1 = newTrackJoint(0, 0, (90 / 180) * Math.PI);
  ...
  it('not aligned on z axis', () => {
    const j1 = newTrackJoint(10, 0, (90 / 180) * Math.PI);
  ...
  it('aligned on angle', () => {
    const j1 = newTrackJoint(0, 0, (30 / 180) * Math.PI);
