...
export class RailMapCreator {
    static create(store: Store): RailMap {
        ...
        map.addNodes(store.getAllOf(TYPES.PathBlock) as PathBlock[] as RailMapNode[]);
        ...
        for (let pathBlock of pathBlocks) {
            for (let end of pathBlock.getPathBlockEnds()) {
                ...
                while (true) {
                    ...
                    nodes.push(...filteredMarkers.map(m => m.marker.platform.getStation()));
                    console.log('track parts', start.getTrackParts());
                ...
                if (nextOtherEnd) {
                    ...
                    console.log("NODES", nodes.map(n => n.getName()));
                    for (let i = 1; i < nodes.length; i++) {
                        map.addEdge(nodes[i - 1], nodes[i], 0.5);
                    }
                } else {
                    console.log('DEADEND', oneAheadEnd?.getHash());
                    // todo handle deadends as well
                    console.log("NODES", nodes.map(n => n.getName()));
                    for (let i = 1; i < nodes.length; i++) {
