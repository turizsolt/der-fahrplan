...
export default class RailMapShower extends Vue {
  ...
  init() {
      ...
      for(let i=0;i < this.nodes.length; i++) {
        ...
        this.nodes[i].textDisplay = this.nodes[i].getType() === TYPES.Station ? 'block' : 'none';
        this.nodes[i].bgColor = this.nodes[i].getType() === TYPES.Station ? 'aqua' : 'gray';
      }

      for(let i=0;i < this.edges.length; i++) {
        const coord = this.edges[i].from.getCoord().midpoint(this.edges[i].to.getCoord());
