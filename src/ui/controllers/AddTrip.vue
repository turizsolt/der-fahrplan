<template>
  <div>
    ...
    <div>
      <div class="trip-list">
        <div
          :class="trip.type === 'Trip' ? 'trip' : 'trip-as-group'"
          :key="trip.id"
          v-for="trip in filteredTripList"
        >
          <div v-if="trip.type === 'Trip'">
            ...
            <div class="trip-stop trip-id-header trip-id-divider">
              ...
              <div
                class="trip-repeat-adder"
                @click="addRepeatedTrips(trip.id, trip.nextId)"
              >
                +
              </div>
            </div>
          ...
          <div v-if="trip.type === 'TripGroup'" class="trip-group">
            <div v-if="closedGroups[trip.id]" class="trip-group-headway">
              <b>{{ Math.floor(trip.headway / 60) }}</b
              >{{ fraction(trip.headway % 60) }}<br />
              mins<br /><br />
            ...
            <div
              ...
              v-else
              :key="trip2.id"
              v-for="trip2 in trip.trips"
            >
              <div
                class="trip-stop trip-id-header blue"
                @click="paste(trip2.id)"
              >
                {{ trip2.prevTrip }}
              </div>
              <div class="trip-stop trip-id-header trip-id-divider">
                <div class="trip-id">{{ trip2.id }}</div>
                <div class="trip-repeat-adder" @click="collapseGroup(trip.id)">
                  -
                </div>
              </div>
              <div class="trip-stop">
...
export default class AddTrip extends Vue {
  ...
  get filteredTripList() {
    const list = this.tripList.filter(
      (t) => t.routeId === this.route.id && !t.hasGroup
    );
    list.push(
      ...this.tripGroupList.filter(
        (t) => t.trips[0] && t.trips[0].routeId === this.route.id
      )
    );
  ...
  addTrip(): void {
    const route = getStorable(this.route.id) as RouteVariant;
