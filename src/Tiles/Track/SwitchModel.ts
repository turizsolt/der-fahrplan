...
export class SwitchModel extends TrackModel {
    public static getSleeperVector(track: SwitchModel, sleeper: number): Vector {
        if(track.isSwitched()) {
            // turning
    ...
    public static doSwitch(track: SwitchModel):SwitchModel {
        return new SwitchModel(track.type, track.id, track.center, track.box, track.sleeperCount, track.attachedA, track.attachedS, track.attachedB, !track.switched);
    }
